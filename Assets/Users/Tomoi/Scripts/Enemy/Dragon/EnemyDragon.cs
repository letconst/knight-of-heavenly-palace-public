...
public class EnemyDragon : EnemyBase, IDamageable
{
    ...
    [SerializeField,Header("攻撃時、下方向にプレイヤーが居るか判定するときの円錐の角度")]
    private float AttackConeAngle = 15f;
    ...
    void Update()
    {
        ...
        //float f = Vector3.Dot(_player.position.normalized, transform.position.normalized);
        //Debug.Log(f );
        //Debug.Log(f < 0);
        if (!playerIsInRange)
        {
...
/*
    ...
    private async UniTaskVoid c_Attack(CancellationToken _token = default)
    {
        ...
        look = Quaternion.LookRotation(aim, _mouthPosition.up);
        //look = Quaternion.Euler(aim.normalized);

        Debug.Log(aim);
        Debug.Log(look);

        Debug.DrawLine(_mouthPosition.position, _mouthPosition.position + aim, Color.red, 5, false);
        //TODO:攻撃モーション
        //1秒待機
...
*/
    ...
    private async UniTaskVoid c_Attack(CancellationToken _token = default)
    {
        ...
        Quaternion look;
        Vector2 tmpPlayerV2, tmpDragonV2;
        //判定を 距離 -> プレイヤーとの角度 に変更
        //角度の計算
        // ベクトル   原点               対象の座標
        // vec    = player.position - transform.position
        // 判定 Mathf.Abs( vec.x) <= 円錐の角度 || Mathf.Abs( vec.y) <= 円錐の角度 
        //
        //ドラゴンから下方向に円錐状の判定を作成し、また、ドラゴンからプレイヤーを見たときの角度が一定値以内になるまでループする
        vec = _player.position - transform.position;
        while (
            //下方向の判定
            Mathf.Abs( vec.x) <= AttackConeAngle || Mathf.Abs( vec.y) <= AttackConeAngle
            //横方向の判定
